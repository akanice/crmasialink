...
class Ajax extends MY_Controller{
    ...
    public function favortour(){
        ...
        $this->load->model('tourslistingmodel');
        $tour = $this->tourslistingmodel->read(array('id'=>$id),array(),true);
        if (!$tour){
            $result->msg = 'Dữ liệu không hợp lệ';
            $result->reload = true;
            echo json_encode($result);die();
        ...
        $favortours = $this->session->userdata('favortours');
		$site_url = site_url();
        if (!$favortours) $favortours = array();
        if (isset($favortours[$tour->id])){
            $result->msg = 'Bạn đã thêm địa điểm yêu thích này rồi';
            $result->reload = true;
            echo json_encode($result);die();
        }
        $favortours[$tour->id] = $tour->id;
        $this->session->set_userdata('favortours',$favortours);
        $result->ok = true;
        $result->msg = 'Ok';
        $result->tour = $tour;
        $result->count = count($favortours);
        $result->html = "<tr><td>".count($favortours)."</td><td><a href='".$site_url."tours/chi-tiet/".$tour->alias."'>".$tour->name."</a></td></tr>";
        echo json_encode($result);die();
    }
}
